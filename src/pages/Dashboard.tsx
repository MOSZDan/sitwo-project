    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [u, p, c] = await Promise.all([
          ...
          Api.get<{ count?: number; results?: any[] }>("/pacientes/?limit=1").catch(() => ({ data: { results: [] } } as any)),
          Api.get<{ count?: number; results?: any[] }>("/consultas/?limit=1").catch(() => ({ data: { results: [] } } as any)),
        ]);
        setCounts({
        ...
        <section className="mt-6 sm:mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          >
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-cyan-100 grid place-items-center shrink-0">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 11V3a1 1 0 012 0v8h3l-4 4-4-4h3zM5 19h14" />
                </svg>
              </div>
              <div className="min-w-0">
